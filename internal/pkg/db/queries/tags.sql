...
-- Returns up to 100 of the user's tags matching the given LIKE pattern together
-- with the number of the user's tracks that carry each tag. Tags with no tracks
-- are included with a count of zero. Ordered by track count desc, then tag asc.
...
-- Returns up to 100 tag names that appear on public tracks (grouped by tag
-- string across all users), together with the count of distinct public tracks
-- carrying each tag. Filtered by the given LIKE pattern. Ordered by count
-- desc, then tag asc.
