...
-- Remove all existing rows so we can safely recreate the tables.
-- The downloader jobs will repopulate the data on their next run.
DELETE FROM road_closures;

DROP TABLE road_closure_cells_res7;
DROP TABLE road_closures;
...
CREATE TABLE road_closures (
    ...
    -- type is the closure kind (1 = closed_way, 2 = detour).
